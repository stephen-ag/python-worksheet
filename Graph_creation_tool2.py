...
#    value1 =value.cumsum()
    ...
    df1["YTD Revenue"]=df1['Revenue_SM ($)'].cumsum()
...
ax22.plot (df_plot1.index,df_plot1.iloc[:,numbers+3],label='YTD cost %',\
           marker='o')
ax2.bar (df_plot1.index,df_plot1.iloc[:,0],label= name,\
                                 edgecolor ='black', color = 'cornflowerblue')
#cornflowerblue
ax2.bar (df_plot1.index,df_plot1.iloc[:,1],label=name,\
...
ax1.plot(df_plot1.index,df_plot1.iloc[:,numbers],label='Total Cost',\
         marker ='s',markersize=15, color = 'brown') 
...
for x,y in zip(df_plot1.index,df_plot1['Total Cost']):
    ...
    ax1.annotate(label, (x,y),textcoords="offset points",                    
                        xytext=(-10,15),ha='center',fontsize = 12,color='red')
    ax1.grid(axis ='x')
 #---------------------annotation for YTD %------
